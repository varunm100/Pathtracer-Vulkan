...
static RtConfig rt_config { .sample_count = 3, .max_bounce = 5, .exposure = 1.0f, .num_lights=1, .frame_count = 0, .show_lights = true, .enable_skybox=false};
...
void draw_gui(RtProgram& program, Camera* camera) {
  ...
  ImGui::Checkbox("Enable Skybox", &rt_config.enable_skybox);
...
void run(GLFWwindow* window) {
  Scene scene;
  std::string filename = "../../../scenes/staircase.scene";
